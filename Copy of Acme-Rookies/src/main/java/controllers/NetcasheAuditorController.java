...
@Controller
@RequestMapping("/netcashe/auditor")
public class NetcasheAuditorController extends AbstractController {

	@Autowired
	NetcasheService	quoletService;
	...
	public ModelAndView list(@RequestParam final int auditId) {
		ModelAndView result;
		Collection<Netcashe> netcashesAll;

		final Collection<Netcashe> netcashes = new ArrayList<>();
		...
		try {
			...
			netcashesAll = this.auditService.findOne(auditId).getNetcashes();

			for (final Netcashe x : netcashesAll)
			...
			result.addObject("requestURI", "netcashe/auditor/list.do");
