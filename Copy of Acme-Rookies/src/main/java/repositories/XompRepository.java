...
public interface XompRepository extends JpaRepository<Xomp, Integer> {
	...
	@Query("select avg(1.0*(select count(r) from c.xomps r where r.draftMode=false)) from Audit c")
	Double avgNumberXompsPerAudit();

	@Query("select stddev(1.0*(select count(r) from c.xomps r where r.draftMode=false)) from Audit c")
